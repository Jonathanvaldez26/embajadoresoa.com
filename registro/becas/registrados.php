											...
											<div class="col-md-8 ">
												<form id="form1" name="form1" method="post" action="<?php echo $PHP_SELF?>">
			                      				...
			                      				<table class="table table-bordered table-striped mb-0" align="center">
                            						...
                            						<?php 
                            						$ssql="SELECT id_registrado, prefijo, CONCAT( `apellidop` , \" \", `apellidom` , \" \", `nombre` ) AS nombrecompleto, email,socio FROM registrados WHERE"; 
                            						if($_POST[nombres]<>''){
                            						...
                            						if(!isset($_POST[nombres]) || $_POST[nombres]==''){
                              							$ssql="SELECT id_registrado, prefijo, CONCAT( `apellidop` , \" \", `apellidom` , \" \", `nombre` ) AS nombrecompleto, email, activo FROM `registrados` WHERE id_registrado=0"; 
                            						...
                            						<?php }else{
                              						...
                              						<tr> 
	                                					...
	                                					<td align="center" nowrap>   
	                                						<p align="left"><strong><?php echo $reg['email'];?></strong></p>
	                                					</td>
                                						<td align="center" >
                                 							<?php 
	                                 							$sqlbeca="SELECT * FROM laboratorios l INNER JOIN becas b ON l.id_laboratorio = b.id_laboratorio WHERE b.usadopor =".$reg['id_registrado'];
	                                 							...
	                                 							if ($reg['socio']==1) {
	                                 								$leyendabeca="Socio Activo, no se puede becar";
	                                 							}else{
	                                 								if (empty($resbeca)) {
													...
													<div class="form-group row">
														...
														<div class="col-lg-6">
															<select id="beca" name="beca" class="form-control">
																<option value="">(Seleccionar)</option>
												              <?php
												              $sbecas="SELECT * FROM becas WHERE usadopor = 0 AND id_laboratorio=".$_SESSION['becas']['id'];
									...
									<table class="table table-bordered table-striped mb-0" id="datatable-tabletools">
										...
										<tbody>
											...
											foreach ($becas as &$beca) {
												if ($beca['usadopor']!=0) {
													$sql_usado="SELECT * FROM registrados r INNER JOIN becas b ON b.usadopor = r.id_registrado WHERE r.id_registrado =".$beca['usadopor'];
											...
											<tr>
												...
												<td><?php if ($beca['usadopor']==0) { echo "SI";}else{ echo "NO";} ?></td>
												<td><?php 
												if ($usado==1) {												
												 echo $registrado['nombre']." ".$registrado['apellidop']." ".$registrado['apellidom']; }else{ echo "SIN USAR"; }?></td>
												<td><?php 
												if ($usado==1) {												
												 echo $registrado['email']; }else{ echo "SIN USAR"; }?></td>
												<td>
													<?php if ($usado==1) { ?>
										...
										<div class="col-lg-10" style="margin: 0 auto;">
											<form class="form-horizontal form-bordered" autocomplete="nope" action="" method="POST" onsubmit="return false;" accept-charset="utf-8"  name="ajax-form" id="ajax-form">
												...
												<div class="form-group row">
													<label class="col-lg-3 control-label text-lg-right pt-2" >Apellido Materno</label>
													<div class="col-lg-6">
														<input type="text" class="form-control" value="<?php echo $registrado['apellidom'] ?>"  id="apellidom-edit" name="apellidom-edit"  style="text-transform: uppercase;">
													</div>
												</div>												
												...
												<div class="form-group row">
													<label class="col-lg-3 control-label text-lg-right pt-2" >Correo</label>
													<div class="col-lg-6">
														<input type="text" class="form-control" value="<?php echo $registrado['email'] ?>" id="correo-edit" name="correo-edit" >
													</div>
												</div>
		...
		<script type="text/javascript">
			...
			$(function() {
		      $("#btn-registro").click(function(){
		         if($("#nombre").val()!="" && $("#apellidop").val()!="" && $("#apellidom").val()!="" && $("#correo").val()!="" && $("#pais").val()!="" && $("#beca").val()!=""){
