...
<body>
  ...
  <div id="content-wrap" >
    ...
    <main class="row" style="width:90%; margin:0 auto; background-color: #fff; padding: 25px;" >
      ...
      <div class="row">
        <table id="example" class="table" style="width:100%">
          ...
          <tbody>
            ...
            $query="SELECT * FROM preguntas p INNER JOIN registrados r ON p.id_registrado=r.id_registrado INNER JOIN paises pa ON r.id_pais=pa.id_pais INNER JOIN estados e ON r.id_estado = e.id_estado ORDER BY p.fecha DESC ";
            ...
            ?>
              ...
              <tr>
                <td><?php echo $i; ?></td>
                <td><?php echo utf8_encode($registrado['pregunta']); ?></td>
                <td><?php echo utf8_encode($registrado['nombre']." ".$registrado['apellidop']." ".$registrado['apellidom']); ?></td>
                <td><?php echo utf8_encode($registrado['pais']); ?></td>
                <td><?php echo utf8_encode($registrado['estado']); ?></td>
      ...
      <?php 
      $sql="SELECT *FROM sesionesasistentes as sa INNER JOIN registrados as s on s.id_registrado=sa.id_registrado WHERE sa.id_sesion=1 group by s.id_registrado order by id_pais, id_estado";
      $total_asistentes=$bd->RecordCount("sesionesasistentes","id_sesion = 1");
   ...
   foreach ($asistentes as &$asistente) {
      ...
      <div class="row">
        <table class="table table-hover">
          <thead><th>Pais</th><th>Visitantes</th><th>Porcentaje %</th></thead>
            ...
            <tr>
              <td>
                <?php echo utf8_encode($pa[pais])?>
              </td>
              <td>
                <?php echo count($buscar)?>
              </td>
              <td>
                <?php echo $porcentaje=(count($buscar)*100)/$total_asistentes?>%
                <?php 
    ...
    foreach ($ciudadesdif as $ciudad) {
        ...
        $buscar = (array_keys($ciudadestot, $ciudad));
          ...
          <tr>
            <td>
              <?php echo utf8_encode($ci[estado])?>
            </td>
            <td>
              <?php echo count($buscar)?>
            </td>
            <td>
              <?php echo $porcentajec=(count($buscar)*100)/$total_asistentes?>%
              <?php 
